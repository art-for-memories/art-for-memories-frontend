...
const GalleryTable: React.FC<{
    ...
    onPreview?: (Stories: Gallery) => void;
    onApproved?: (id: string) => void;
}> = ({ headers, data, onDelete, onPreview, onApproved }) => {
                ...
                <tbody>
                    ...
                    {data.map((Stories) => (
                        <tr key={Stories.id} className="focus:outline-none h-16 border border-gray-100 rounded-lg bg-white text-black text-sm">
                            ...
                            <td className={`p-4 max-w-[200px] overflow-hidden ${selected.includes(Stories.id) ? 'font-bold' : ''}`}>
                                {Stories.image}
                            </td>
                            <td className="p-4">
